...
void VulkanEngine::cleanup()
{
    if (_isInitialized) {
        ...
        // destroy the rectangle mesh buffers
...
void VulkanEngine::create_swapchain(uint32_t width, uint32_t height)
{
    ...
    VkPresentModeKHR presentMode = _enableVsync ? VK_PRESENT_MODE_FIFO_KHR : VK_PRESENT_MODE_IMMEDIATE_KHR;
    ...
    // Create semaphores for each swapchain image to avoid reuse issues
...
void VulkanEngine::init_swapchain()
{
    ...
    // set the draw image format to match the compute shader (R16G16B16A16_SFLOAT)
...
{
    // Create the pipeline layout first with push constants
...
void VulkanEngine::draw()
{
    ...
    // Use per-frame semaphore for acquire, then use per-swapchain-image semaphores for rendering
    ...
    // Now reset the fence after we know we will submit work
    ...
    // Apply render scale to draw extent
    _drawExtent.width = static_cast<uint32_t>(_drawImage.imageExtent.width * renderScale);
    _drawExtent.height = static_cast<uint32_t>(_drawImage.imageExtent.height * renderScale);
    ...
    // Wait on per-frame acquire semaphore, signal per-swapchain-image render semaphore
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd) {
    ...
    // sort opaque drawing by material and mesh
    ...
    *sceneUniformData = sceneData;
    ...
    for (auto& opaqueDrawIndex : opaque_draws) {
        draw(mainDrawContext.OpaqueSurfaces[opaqueDrawIndex]);
    }
...
void VulkanEngine::init_triangle_pipeline()
{
    ...
    // set depth format to match the depth image, even though we won't use depth testing
